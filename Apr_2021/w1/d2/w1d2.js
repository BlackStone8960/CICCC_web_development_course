...
const crew = spaceship7.crew;
for (let prop in crew) {
  console.log(`${prop}: ${crew[prop].name}`);
}
...
for (let prop in crew) {
  console.log(`${crew[prop].name}: ${crew[prop].degree}`);
}
...
console.log(robot5.provideInfo());
...
//3) If this._energyLevel isn’t a number it could be that the _energyLevel property was altered. 
...
//1)Currently, in robot there is a getter method for numOfSensors but no setter method! 
